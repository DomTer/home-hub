...
const csvRowToObjectGiro = function (row) {
    return {
        ...
        date: row.Buchungstag ? formatDate(row.Buchungstag) : formatDate(row.Valutadatum),
...
const csvRowToObjectCreditcard = function (row) {
    let prepared = {
        ...
        date: row.Belegdatum ? formatDate(row.Belegdatum) : formatDate(row.Buchungsdatum),
        ...
        raw: JSON.stringify(row),
        hash: 'undefined', // Hash now will fail because of bookingRef
    }
    const bookingRef = prepared.bookingRef;
    const raw = prepared.raw;
    delete prepared.raw;
    delete prepared.bookingRef;
    delete prepared.hash;
    const hash = md5(JSON.stringify(prepared));
    prepared.bookingRef = bookingRef;
    prepared.raw = raw
    prepared.hash = hash
    return prepared
}
...
const insertToTransactionsCreditcard = function (values) {
    let db = new Database();
    return db.query("INSERT IGNORE INTO transactions_creditcard (text, text2, date, valuta, exchangeRate, value, currency, income, bookingRef, raw, hash ) VALUES ?",
